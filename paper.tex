...
Prevailing approach to the definition of valid syntax of programming languages is through/using grammars written, for example, using notations based on (or similar to) EBNF [reference]. Many popular tools exist for automated generation of parsers from the grammars, such as ANTLR and Bison/Flex. Source code of programs in the language is then written in a text editor, which is a part of some IDE.

An alternative emerged recently in the form of projectional editors that are used mainly in special IDEs for domain specific languages, which are called language workbenches. A popular tool of this kind is MPS [odkaz] developed by JetBrains. In MPS, you can design a custom language, and then use it as an IDE for writing programs in the language and compiling them into executables. The key idea behind projectional editors is that a developer directly manipulates the AST representation of the source code, and not a plain text. \todo{Kratce zminit vyhody a nevyhody toho postupu.} One advantage of projectional editors is that developers practically cannot make syntax errors (MPS IDE will not allow them to make syntax errors). Furthermore, languages can be defined in a modular way (object-oriented, hierarchical) and multiple languages can be easily combined together. 
The mechanisms supported by MPS also give us, for instance, the possibility to extend existing languages very easily, just by inheritance.
Nice illustration of the abilities of MPS is the mbeddr project [citation] which extends C with domain-specific constructs and data types.
...
In order to use a language inside MPS, it has to be defined using the MPS concepts. In particular, one has to create the structure (types of AST nodes), editor (projection of AST), and text generation (to enable creation of plain text files used as input for compilers). We provide more details in Section X (Background about MPS).

However, it is a lot of work to create a definition of a programming language in MPS. Therefore, very few mainstream languages are now fully supported by MPS (through some extensions).
\todo{From thesis.
...
\todo{From thesis.
	We will look into the possibility of automatic import of already existing general purpose languages into MPS using a grammar description of their syntax.
}
...
JetBrains MPS (Meta Programming System) is a development environment that allows building/creating/defining custom (domain specific) languages or extending existing ones.
Developers can use the newly created languages inside MPS, and for their programs they can also instruct MPS to generate code in a given target language such as Java that is compiled to make an executable.

We need to provide a good understanding of how the MPS~\cite{ref:MPS} environment works.
Focus on basic/fundamental and relevant features.
In this section, we will explain fundamental basics of MPS as it is crucial in the understanding of our contribution and associated challenges.

MPS differs from mainstream/typical IDEs in one important aspect.
In MPS, user/developer does not work with the textual representation of the source code directly, but rather with its AST (abstract syntax tree) that is the model of the code.
The code itself, which is later compiled and run, is built out of this tree.
When users are creating programs using MPS and projectional editor, they are basically assembling the tree (AST) out of predefined building blocks of the language.
The definition of the language dictates, which statements or elements can be nested inside each other and what structure the AST can have.

Main building blocks of any MPS language are nodes of the AST.
Code of a program is built from the AST nodes.
...
Another advantage is that every node of the AST can be represented in any visual form and take any shape.
...
The type of nodes can be restricted by the language structure definition. For example, we can restrict the condition in the if/else statement to be an expression, statement block to be a list of statements, etc.

Alternative representations are possible, and they are not bound to be just the textual representation at all.
For example, blocks of both branches can be aligned next to each other like shown in Figure~\ref{fig:if_editor}.
...
The definition of a language element/construct (a type of an AST node) consists of several aspects where each aspect represents a different point of view.
The most important aspects are the following: structure (basic properties, where the node can appear in the AST (tree)), editor (how does it look like, how it is projected/presented), textgen (what code in the underlying base language is generated out of it).
Editor, textgen, and other aspects of a language (see below) must be defined in order to make the language comfortably usable inside MPS.

The three most important aspects (components) of a language definition in MPS are the following: structure (AST nodes and their possible relationships/hierarchy/tree), editor, text generation. They are also most relevant for the contribution of this paper, so we describe them here (and neglect/ignore the other aspects such as type system and data-flow).
...
Some concepts and design patterns known from object oriented programming can be used in the language definition --- inheritance, interfaces, and so on.
The AST node can be inherited from a parent (abstract) node and it can implement a special interface node.
Most importantly, nodes have child nodes and that is how the AST itself is created.

\todo{Zduraznit ze AST node (object) ma nekolik aspectu: structure, editor, textgen. Definice MPS language je organizovana takto --- pres AST nodes, ktere maji nekolik aspectu (ruzne pohledy).}

\paragraph{Structure.}
The most important aspect of any MPS language is structure. It must be created first for each AST node.
It specifies core attributes such as name of the AST node, inheritance relationships, possible child nodes (types and cardinality), implemented interfaces, references to other nodes, other properties (fields of any type that can hold values), etc.
...
The editor aspect of an AST node defines its visual representation (of the code) in the projectional editor.
...
A program written in an MPS language cannot be executed directly.
...
MPS supports also other aspects, such as type checking and data-flow analysis, but they are not important for our contribution.
Moreover, information needed for their automated generation is not available/contained inside a grammar file.
