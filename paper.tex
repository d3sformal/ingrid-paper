...
\begin{abstract}
\PV{Manipulate language definitions mozna uplne neevokuje tvorbu jazyku. Slo by to rict vic polopaticky? Tady mozna pomuze vic Vaclav.}
\JB{"Manipulate language definitions" - navrhuji "create or modify language definitions"}
JetBrains MPS is a language workbench, an IDE that allows developers both to write code and manipulate language definitions.
...
While MPS focuses on domain-specific languages (DSL), it needs to support also general-purpose programming languages (GPLs) --- for example, to compile and run programs written in some DSL, or to enable combination of GPLs with \JB{Navrhuji nahradit "languages" za "DSLs"} languages created in MPS.
