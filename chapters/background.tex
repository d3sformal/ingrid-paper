...
In this section, first we introduce basic features of the MPS platform, and then we discuss related work, including other existing projects that aim to add support for general purpose languages into MPS \todo{IDE based on projectional editor}.

\subsection{JetBrains MPS}
\label{sect:MPS}

\todo{For each MPS term (Structure, Editor, TextGen, concept, and so on) say which generally/widely known terms (from related academic work) are similar/analogous. Relate the MPS-specific terms to the large body of academic work. Structure je abstract syntax, Editor je concrete syntax --- ale to uz rikame.}
\todo{Mozna rict jak se zakladni aspekty (Structure, Editor, TextGen) nazyvaji v jinych IDE (MetaCase MWB, apod).}
...
\todo{Mozna pouzit termin "language concept" misto samotneho "concept" - ale jen pri definici.}
...
\todo{zvyraznit slovo "concept"} In the context of MPS, a concept is, in fact, \todo{a language element or} a building block of a language definition.
...
DSLs, on the other hand, need to define rules for model-to-model conversions (Generators), since \todo{programs in these languages} are rarely converted to text directly.
...
\todo{We are aware of only few attempts to create ports of general-purpose languages into (IDEs based on) projectional editors/editing. All of them were conducted manually.}
...
\todo{Pro vsechny nasledujici projekty (PE4MPS, ANTLR{\_}MPS, mps{\_}metabnf) mention GPPLs for which there projects have been used (e.g., JavaScript). We discuss application of PE4MPS on JavaScript in Section 4.}
...
\todo{Check if there is a new (more recent) version with more features.} The current version, as of November 2016, supports just horizontal lists, vertical lists, and some indentation rules.
...
\todo{Najit dalsi podobne systemy a kratce popsat existujici reseni podobnych uloh (automated import of general-purpose languages) v jejich kontextu.}
...
S. Erdweg, T. Rendel, C. K\"{a}stner, and K. Ostermann. Layout-Sensitive Generalized Parsing. In Proceedings of SLE 2012, LNCS 7745.
	...
	The main difference from our approach (INGRID): No layout annotations in the grammar are used. Currently, simple heuristics are used to create editor and textgen (i.e., to determine layout).
}

\todo{
\paragraph{Other IDEs.}
There also exist several language workbenches that are based on parser generators (such as ANTLR) and allow free editing of textual source code (unlike the projectional editor in MPS).
A prominent example/tooo in this category is Spoofax [ref].
	Reference: L.C.L. Kats and E. Visser. The Spoofax Language Workbench: Rules for Declarative Specification of Languages and IDEs. OOPSLA 2010, ACM.
	Syntax of DSLs is essentially restricted to linear text.
INGRID is not applicable to such tool/workbenches (for creating languages inside them).
}
