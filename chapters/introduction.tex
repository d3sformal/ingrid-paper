...
Before a program written in a rich DSL can be executed, it is \todo{has to be} transformed on the AST level by a series of model-to-model transformations to an AST model that represents the desired semantics in a general-purpose language (GPPL), such as C or Java.
...
However, very few mainstream GPPLs are now fully supported by MPS \todo{(and other language workbenches based on the same principles, such as MetaCase MetaEdit+ WorkBench (MWB) \url{http://www.metacase.com/mwb/})}, because it requires substantial effort to manually create a full definition of a GPPL in MPS.
...
\todo{Although we define the INGRID method in the context of MPS, it can be easily adapted for/towards construction of language definitions usable in (suitable for) other IDEs and language workbenches (i) based on projectional editing (which let the user edit the AST representation (abstract syntax) directly) and (ii) where language definition is based on structural object-oriented approach.}
