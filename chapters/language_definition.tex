...
Unlike some of the related projects, we did not extend the notation with any custom features, and we also did not create an MPS language for the ANTLR notation \PV{Mozna vysvetlit proc (protoze je to dvoji prace) nebo odkazat tam, kde je to vysvetleny?}.
...
\PV{Nasledujici odstavec jsem lehce vylepsil, asi je to ted vysvetleno lepe.}
The first phase in the process of MPS language construction is parsing of the input ANTLRv4 grammar.
This is done using an automatically generated ANTLRv4 parser that was created from the grammar of the ANTLRv4 notation itself.
The full tree, which comes out of the parser is, however, quite complex and contains information not relevant for the INGRID algorithm.
...
An output of this phase is a custom tree-like structure representing the grammar.

\PV{Tenhle dalsi odstavec bych klidne celej odebral, neprinasi zas tolik informaci, vzhledem k tomu, ze nejde do hloubky.}
A simplified tree of objects is derived in two passes over the grammar.
...
\PV{Tohle se mi libi, jak si prepsal, asi neni co vylepsit.}
...
\PV{Tady bych na zacatek vety pridal neco jako "In MPS...". Neni jasne, jestli mluvime o necem v ingrid nebo o konceptech MPS.}
...
\PV{Dalsi vetu bych nahradil: }
\PV{Values of these properties will be restricted using the regular expression that corresponds to the \antlrlexerrule{Name} lexer rule.}
\PV{Protoze }
...
\PV{Mozna "for" nahradit necim z XML, treba zavorkami </>, najednou totiz se preskoci na for, ktery se vzal odnikud.}
...
\PV{Label CONTENTRULE defined multiple times! Pak to ukazuje spatny cislo Figure}
...
\PV{Odstranil bych ", i.e. for each AST node with more than one child"}
...
\PV{K Phase 3 nemam vyhrady, to je prepsano super.}
...
\PV{Nejsem si jisty, jestli to nebylo az pozdeji, ale byl tu nekde odstavec, ktery se zminoval o tom, ze whitespace je silent-polykan parserem a proto pravidla nemusi obsahovat explicitne informaci o tom, kde vsude muze byt mezera \\\texttt{'<' WS* Name WS* attribute* WS* '>' WS*...}.\\To podle me dobre ilustrovalo, ze se tam ztraci ta informace. Prislo mi to jako zajimave poukazani, ale samozrejme to tu byt nemusi, pokud se ti zda, ze to az tak zajimave neni.}
...
The second example relates to a fragment of the JavaScript language, also known as ECMAScript\footnote{https://github.com/antlr/grammars-v4/blob/master/ecmascript/ECMAScript.g4}.
