...
The key idea of this approach is to avoid all the intermediary layers (nodes) in the syntax tree by considering just the leaf nodes.

% TODO put here the example from below

When given a particular AST node $n$ (or a rule of the grammar) as input, the parser tree (AST) built in the earlier phases is systematically traversed in order to identify each AST node that may appear at the end of a derivation chain from the input node $n$.
...
\todo{Shortcut approach differs in the way references to children (parser rules) are created.}
\todo{The main/key difference between these two lies in the way we create children fields for parser rules and in the way we are going to link them together using interface concepts.}

The key idea of this approach is to identify rules and nodes that are at the end of the (derivation, inheritance) chain
...
End nodes and rules are then directly offered to the user through the auto-completion menu.
