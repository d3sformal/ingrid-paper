...
When designing the process for transforming AST nodes (i.e., grammar rules) into the MPS language structure, we faced several challenges.
...
Such rules exist at the intermediate layers of a syntax hierarchy, and their main purpose is to enable easier understanding of the grammar by humans.
We show examples illustrating this problem later, in the subsections where we describe our solution.
Nevertheless, presence of the intermediate layers significantly complicates usage of the given language in MPS, and the layers are actually not necessary for construction of an MPS language.

We have designed two approaches to eliminate the unnecessary layers during the process of creating the Structure aspect --- the \emph{straightforward} approach (Section~\ref{sect:STRAIGHTFWD}), and the \emph{shortcut} approach (Section~\ref{sect:SHORTCUT}) that is more complex and builds upon the first one.
