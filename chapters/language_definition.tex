...
Syntax definition in the form of an ANTLR v4 grammar exists for all widely used programming languages~\footnote{https://github.com/antlr/grammars-v4}.
...
The process of language construction by \textsc{Ingrid} consists of four phases --- the first is parsing of the input grammar, followed by definition of the essential aspects of the language (Structure, Editor, and TextGen, respectively).
...
We will describe the \textsc{Ingrid} method using a simplified XML language defined in Figure~\ref{fig:SIMPLEXML}.
...
Nevertheless, the full parse tree, which comes out of the parser, is quite complex and contains information not relevant for the \textsc{Ingrid} algorithm.
In order to get a simple representation that is easy to process by the later phases and keeps only information necessary for the construction of MPS languages, several steps of post-processing of the parse tree are performed in this phase.
...
For each lexer rule, the parser produces a tree that captures its structure.
The lexer rules are, in fact, just regular expressions used to recognize tokens in the input string.
...
Our implementation of the algorithm detects rules defined in a recursive way (e.g., using the pattern \antlrlexerrule{N} : \antlrliteral{text} \antlrlexerrule{N}) and rejects the input grammar if it contains such rules.
...
Flatten(\textit{R}):
  ...
  \textbf{for each} alternative \textit{A} \textbf{in} rule \textit{R}:
    ...
    \textbf{for each} element \textit{E} \textbf{of} \textit{A}:
      \textbf{if} \textit{E} is not yet flattened \textbf{then} Flatten(\textit{E})
...
Specifically, the \antlrparserrule{content} rule from the SimpleXML grammar, given in Figure~\ref{fig:CONTENTHIERARCHY} together with rules that determine the relevant fragment of the syntax hierarchy, expands ultimately into the nodes highlighted using the bold font in Figure~\ref{fig:CONTENTEXPAND}.
...
\small
  \antlrparserrule{content} : \antlrlexerrule{TEXT} | \antlrparserrule{element} | \antlrparserrule{comment} | \antlrlexerrule{CDATA} ;
  \antlrparserrule{element} : \antlrliteral{<} \antlrlexerrule{Name} \antlrparserrule{attr}* \antlrliteral{>} \antlrparserrule{content}* \antlrliteral{</} \antlrlexerrule{Name} \antlrliteral{>} | \antlrliteral{<} \antlrlexerrule{Name} \antlrparserrule{attr}* \antlrliteral{/>} ;
  \antlrparserrule{comment} : \antlrliteral{<!--} \antlrlexerrule{TEXT} \antlrliteral{-->} ;
\end{alltt}
...
For example, the result of this algorithm for the \antlrparserrule{content} rule (Figure~\ref{fig:CONTENTHIERARCHY}) is the list shown in Figure~\ref{fig:CONTENTEXPAND}.
...
Rules forming the grammar only tell what the syntax tree looks like and how the program text is decomposed into AST nodes.
...
Nevertheless, we plan to work on a more automated approach to the definition of Editor aspects in the future, based on machine learning.
