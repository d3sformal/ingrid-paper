...
Such nodes cannot expand further based on any rule in the grammar, and for that reason we call them \emph{end nodes}. \todo{Srovnat pojem "end nodes" vzhledem ke terminal symbols from grammars (jako analogie). Mozna existuje dalsi vhodny termin.}
...
We experimented with several heuristics to derive a useful code layout, but all of them produced rather suboptimal results \todo{Give examples or quantify this result}.
